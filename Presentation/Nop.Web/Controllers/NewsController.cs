        ...
        public ActionResult List(NewsPagingFilteringModel command)
        {
            ...
            model.NewsItems = newsItems
                .Select(x =>
                {
                    ...
                    var newsPictureCacheKey = string.Format(ModelCacheEventConsumer.NEWS_PICTURE_MODEL_KEY, x.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                    newsModel.PictureModel = _cacheManager.Get(newsPictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(newsModel.PictureId);
                        var pictureModel = new PictureModel()
                        {
                            FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                            ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
                            Title = string.Format(_localizationService.GetResource("Media.News.ImageLinkTitleFormat"), newsModel.PictureId),
                            AlternateText = string.Format(_localizationService.GetResource("Media.News.ImageAlternateTextFormat"), newsModel.PictureId)
                        };
                        return pictureModel;
                    });
